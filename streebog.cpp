...
public:
    ...
    int Test(int is_256) {
        ...
        if (is_256) {
            Hash(1, mes1, 504, res512);
            Hash(1, mes2, 576, res512);
        }
        else {
            Hash(0, mes1, 504, res512);
            Hash(0, mes2, 576, res512);
        }
        return 0;
    }
private:
    ...
    void L(vector<unsigned char>& curr) {
        for (int i = 0; i < 8; i++) {
            ...
            for (int j = 0; j < 8; j++) {
                for (int k = 0; k < 8; k++) {
                    // if current bit !=0 => 
                    // tmp += 1 * A[j * 8 + k] in GF(2) <=> xor A[j * 8 + k]
